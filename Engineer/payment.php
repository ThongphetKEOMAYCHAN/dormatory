 ...
 if(!isset($_SESSION['username'])){
  ...
  header("location:login.php");
         ...
         $select="SELECT * FROM `payment` as p INNER JOIN `students` as s ON p.st_id=s.st_id
                                             ...
                                             INNER JOIN `inclass` as nc ON p.in_id=nc.in_id 
                                             where `p_id`='$payget'";
...
if(isset($_POST['payment'])){
     ...
     $register="UPDATE `payment` SET `p_name`='$st_pay',`st_id`='$stid',`t_id`='$st_em',`h_id`='$hid',`c_id`='$cid',`b_id`='$stb_id',`y_id`='$sty_id',`in_id`='$stin',`date`='$date' WHERE `p_id`='$payget'";
          ...
          <div class="card-body">
               <form method="post">
                    ...
                    <div class="form-group col-md-2">    
                         <label>ຈໍານວນ</label>
                         <input type="number" name="money" class="form-control" placeholder="290000" required>
                    </div>
                    <div class="form-group col-md-2">
                         
                         <input type="hidden" name="st_code" class="form-control" value="<?=$stcode?>">
                    </div>
                    <div class="form-group col-md-3">
                         
                         <input type="hidden" name="st_name" class="form-control" value="<?=$stname?>">                 
                    </div> 
                    <div class="form-group col-md-2">
                         
                         <input type="hidden" name="in_class" class="form-control" value="<?=$inclass?>">     
                    </div>
                    <div class="form-group col-md-2">    
                         
                         <input type="hidden" name="st_item" class="form-control" value="<?=$stem?>">
                    </div>
                    <div class="form-group col-md-2">    
                         
                         <input type="hidden" name="st_year" class="form-control" value="<?=$styear?>">
                    </div>
                    <div class="form-group col-md-3">    
                         
                        <input type="hidden" name="st_home" class="form-control" value="<?=$sthome?>">
                    </div>
                    <div class="form-group col-md-3">    
                         
                         <input type="hidden" name="st_class" class="form-control" value="<?=$stclass?>">
                    </div>
                    <div class="form-group col-md-3">    
                         
                         <input type="hidden" name="st_bed" class="form-control" value="<?=$stbed?>">
                    </div>
                    
                   </div>
                   <button type="submit" name="payment" class="btn btn-primary">ຈ່າຍ</button>
                   </div>
               </form>     
          </div>
        </div>
